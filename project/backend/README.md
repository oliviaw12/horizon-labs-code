...
- `PINECONE_API_KEY` / `PINECONE_INDEX_NAME` (required for ingestion): credentials for the Pinecone vector index.
...
2. Create an index named to match `PINECONE_INDEX_NAME` with:
    ...
    - **Metric:** `cosine` (recommended for OpenAI/OpenRouter embeddings).
